    ...
    return (
        <div className="w-full h-fit flex flex-col gap-2">
            <div className="flex justify-between items-center min-h-7">
                ...
                <Link href={"/plans"} className='text-blue-500 hover:text-blue-700 text-xs duration-300'>See more...</Link>
            </div>

            <div className="h-full flex flex-col gap-2">
                { plans.length === 0 ? (
                    <div className="h-40 flex items-center justify-center border border-neutral-400 rounded-xl p-5 ">
                        <Link href="/plans" className="w-fit border border-neutral-400 font-medium p-3 hover:bg-neutral-100 active:scale-95 rounded-lg duration-300 flex gap-2 items-center justify-center">
                            ...
                            <span className='text-sm leading-3'>Create New</span>
                        </Link>
                    </div>
                ) : (
                    <>
                        {plans.map(plan => (
                            <div key={plan.plan_id} className="rounded border border-gray-400 bg-white p-2">
                                <div className="flex flex-col gap-2">
                                    <div className={`px-1 w-fit rounded-sm text-[10px] leading-none font-semibold text-white ${PLAN_STATUS[plan?.status].color}`}>
                                        <span className="text-center leading-4">{PLAN_STATUS[plan?.status].name}</span>
                                    </div>
                                    ...
                                    { plan?.deadline &&
                                        <div className="flex items-center gap-1 h-3 mt-1">
                                            ...
                                            <span className='text-[10px] leading-3'>{formatDate(plan?.deadline, 'Mon DD, YYYY')}</span>
                                        </div>
                                    }
                                </div>
                            </div>
