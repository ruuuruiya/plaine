...
const RecommendationSection = ({ title, lists, setState }) => {
    ...
    const handleGenerate = async () => {
        try {
            setLoading(title);
            const res = await updateRecommendations(title.toLowerCase());
            if(res.success) {
                setState(res.data);
                setLoading("");
            } else {
    ...
    return (
        <div className="w-full h-fit flex flex-col gap-2">
            <div className="flex justify-between items-center min-h-7">
                ...
                { !loading &&
                    <button onClick={handleGenerate} className="border border-neutral-400 font-medium px-2 py-1 text-center hover:bg-neutral-100 active:scale-95 rounded-md duration-300 flex gap-1 items-center">
            ...
            <div className="h-full border border-neutral-400 rounded-xl p-5">
                { loading === title ? (
                    <LoadingGenerate description={`Generating ${title}`}/>
                ) : (
